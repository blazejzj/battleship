...
class DragDropManager {
    ...
    removePlacedShipsTabIndex() {
        ...
        shipCards.forEach((shipCard) =>
            shipCard.setAttribute("tabindex", "-1")
        );
    }
    ...
    dropIfValid(x, y) {
        ...
        if (map.getAxis() === "X") {
            return [
                map.placeX(new ship(shipOnDrag.name, shipOnDrag.length), [
                    x,
                    y,
                ]),
                shipOnDrag.name,
    ...
    touchMoveHandler(
        ...
        if (
            hoveredElement[0] &&
            hoveredElement[0].classList.contains("field")
        ) {
            this.styleFieldsForDrop(
                fieldContainer,
                this.getNodeIndex(hoveredElement[0])
            );
        }
    }
